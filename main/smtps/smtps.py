...
from smtps.sender import RETURN_RESPONSE_CODE, UNIQ_BLOCK_MX, MERGE_RSP, ADD_TAG_IN_SUBJECT
                ...
                for row_address in row_smtps_rcp['to_addresses']:
                    if CHECK_CONN_SERVER(row_address):
                        if row_address['is_tagged']:
                            result_sendmain = server.sendmail(j_config['sender_module']['header_mail_from'], row_address['to_address'], ADD_TAG_IN_SUBJECT(byte_data,row_smtps_rcp['thresholds']['tag']))
                        else:
        ...
        for row_smtps_rcp in md.smtps_rcp:
            ...
            for row_mx in row_smtps_rcp['mxs']:
                try:
                    ...
                    for row_address in row_smtps_rcp['to_addresses']:
                        if row_address['is_tagged']:
                            result_sendmain = server.sendmail(md.smtpd_mail_from, row_address['to_address'], ADD_TAG_IN_SUBJECT(byte_data,row_smtps_rcp['thresholds']['tag']))
                        else:
