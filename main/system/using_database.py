...
def F_CHECK_QUARANTINE_OR_DELIVERED(data, space):
    if data['smtps_rcp'][0]['to_addresses'][0]['status'] == "quarantined" or \
       data['smtps_rcp'][0]['to_addresses'][0]['status'] == "delivered" or \
       data['smtps_rcp'][0]['to_addresses'][0]['status'] == "rejected":
        ...
        if data['smtps_rcp'][0]['to_addresses'][0]['status'] == "quarantined":
            file_name = "/main/data/quarantine/{}.eml".format(path_file_name)
            log_file_name = "/main/data/quarantine/{}.log".format(path_file_name)
        elif data['smtps_rcp'][0]['to_addresses'][0]['status'] == "delivered":
            file_name = "/main/data/delivered/{}.eml".format(path_file_name)
            log_file_name = "/main/data/delivered/{}.log".format(path_file_name)
        elif data['smtps_rcp'][0]['to_addresses'][0]['status'] == "rejected":
            file_name = "/main/data/rejected/{}.eml".format(path_file_name)
            log_file_name = "/main/data/rejected/{}.log".format(path_file_name)
        else:
            return
...
def ADD_IN_DATABASE_ROW_EMAIL(data: str):
    ...
    if j_data['status'] == "blocked":
        ...
        space_file_binary.delete(list_rows[0]['content_id'])
